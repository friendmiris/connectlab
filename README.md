...
## 7. 카드 문구를 AI로 자연스럽게 다듬기 (무료로 가능해요)

네이버가 주는 요약문은 가끔 라디오 대담 요약처럼 대화체거나 문장 중간에서 잘려있어요. 정규식으로 문장을 나누는 방식은 이런 경우에 어색해질 수밖에 없어서, AI로 자연스럽게 다듬는 옵션을 추가했어요. **두 가지 중 하나를 고르시면 돼요** (Gemini를 추천해요, 완전 무료예요):

### Gemini (추천, 무료)
1. https://aistudio.google.com/apikey 접속 → 구글 계정으로 로그인
2. **"Create API key"** 클릭 → 바로 키 생성됨 (신용카드 등록 불필요)
3. Vercel 환경변수에 `GEMINI_API_KEY` 추가 후 Redeploy

무료지만 사용량 제한은 있어요(분당 10회, 하루 1500회 정도) — 개인 카드뉴스 프로젝트로는 절대 넘을 일 없는 수준이에요.

### Claude(Anthropic) — 유료, Gemini보다 답변 품질이 더 좋을 수 있어요
1. https://console.anthropic.com 에서 가입 → **API Keys** 메뉴에서 키 발급
2. **Billing**에서 결제수단 등록 (건당 1원 미만 수준이라 부담 적어요)
3. Vercel 환경변수에 `ANTHROPIC_API_KEY` 추가 후 Redeploy

**둘 다 넣으면 Gemini가 우선 사용되고, Gemini가 실패하면 자동으로 Claude로 넘어가요.** 아무것도 안 넣으면 지금처럼 문장을 자동으로 나누는 방식이 그대로 쓰여요.

키를 넣으면 카드뉴스 편집 화면을 열 때마다 자동으로 한 번 호출돼서(기사당 1회) 요약문과 포인트를 자연스럽게 다듬어줘요. 뉴스·유튜브 소스 둘 다 적용되고, 실패하면 조용히 기존 방식으로 대체돼요.
